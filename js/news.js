...
onAuthStateChanged(auth, async (user) => {
  ...
  const querySnapshot = await getDocs(collection(db, "news").withConverter(newsConverter));
        querySnapshot.forEach((doc) => {
            const news = doc.data(); 
            createItemNews(news);
            console.log(doc.id, " => ", news.getReName());
        });
...
function createItemNews(news) {
    const node = document.createElement("div");
    const name = document.createElement("h3");
    const content = document.createElement("p");

    node.className="bai_viet";
    name.innerText=news.getReName();
    content.innerText=news.getContent();
...
function manageNews() {
  ...
  document.getElementById("update").addEventListener("click", function(){
    ...
    search.addEventListener("click", async function(){
      const q = query(collection(db, "news"), where("name", "==", e_name.value)).withConverter(newsConverter);
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((docReg) => {
        const newsItem = docReg.data();
        const newsId=docReg.id; 
        console.log(docReg.id, " => ", newsItem);
        edit_item_content.style.display="block";
        e_content.value=newsItem.getContent();
        search.style.display="none";
...
function processRole(employee) {
  if(employee.getRole()=="Admin"){
    document.getElementById("manage_news").style.display="block";
    manageNews();
  }
}
/* End phần Quản lý bang tin */
